...
public class PTra10_04 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < users.length; i++) {
			...
			users[i].userNm = n[1];
			users[i].mail = n[2];
			users[i].passward = n[3];
		}
		//----------- ループ end

		// ★ usersに格納されている全てのインスタンスの、フィールドuserId, userNm, mail, passwordを出力してください
		for (int s = 0; s < users.length; s++) {
			System.out.println(users[s].userId);
			System.out.println(users[s].userNm);
			System.out.println(users[s].mail);
			System.out.println(users[s].passward);
		}
	}
}
